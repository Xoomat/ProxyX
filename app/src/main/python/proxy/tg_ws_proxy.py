...
try:
    ...
    _J_Cipher = jclass("javax.crypto.Cipher")
    _J_SecretKeySpec = jclass("javax.crypto.spec.SecretKeySpec")
    _J_IvParameterSpec = jclass("javax.crypto.spec.IvParameterSpec")
    _HAS_JAVA_CRYPTO = True
except Exception:
    _HAS_JAVA_CRYPTO = False
...
class _AesCtrStream:
    def __init__(self, key: bytes, iv: bytes):
        if _HAS_CRYPTOGRAPHY:
            ...
            self._impl = cipher.encryptor()
            self._available = True
            self._mode = "pyca"
        elif _HAS_JAVA_CRYPTO:
            cipher = _J_Cipher.getInstance("AES/CTR/NoPadding")
            key_spec = _J_SecretKeySpec(bytes(key), "AES")
            iv_spec = _J_IvParameterSpec(bytes(iv))
            cipher.init(_J_Cipher.ENCRYPT_MODE, key_spec, iv_spec)
            self._impl = cipher
            self._available = True
            self._mode = "java"
        else:
    ...
    def update(self, data: bytes) -> bytes:
        if not self._available:
            raise RuntimeError("AES-CTR backend is unavailable")
        if self._mode == "pyca":
            out = self._impl.update(data)
            if not data:
                out += self._impl.finalize()
            return out
        if self._mode == "java":
            if not data:
                return b""
...
def _aes_ctr_keystream(key: bytes, iv: bytes, size: int) -> bytes:
    return _AesCtrStream(key, iv).update(b"\x00" * size)
...
def _is_telegram_ip(ip: str) -> bool:
    if ':' in ip:
        return ip.lower().startswith('2001:67c:4e8:')
    try:
...
def _dc_from_init(data: bytes) -> Tuple[Optional[int], bool]:
    ...
    try:
        key = bytes(data[8:40])
        iv = bytes(data[40:56])
        keystream = _aes_ctr_keystream(key, iv, 64)
...
def _patch_init_dc(data: bytes, dc: int) -> bytes:
    ...
    try:
        ...
        iv = bytes(data[40:56])
        ks = _aes_ctr_keystream(key_raw, iv, 64)
...
class _MsgSplitter:
    ...
    def __init__(self, init_data: bytes):
        ...
        self._dec = _AesCtrStream(key_raw, iv)
